...
ChatBranch is a Chrome/Edge Manifest V3 extension for AI chat navigation and branching workflows.
...
- Not included in current scope:
  - Qwen / Doubao adapters (removed intentionally for stability)

### Core Features

1. **Outline / Index Navigation**
   - Detects user messages from the currently rendered conversation.
...
2. **Branch Ask (New Tab Workflow)**
   ...
   - Builds a branch prompt with:
     ...
     - Full conversation history (current rendered chat)
...
3. **Prompt Library**
   ...
   - Selecting a prompt inserts it directly into the current page composer.

4. **LaTeX Click-to-Copy**
   - Click formula area in AI output (`MathJax` / `KaTeX` / `annotation`) to copy normalized `$...$` LaTeX.

5. **Export Selected Markdown**
   - Export one selected question block (the chosen user question + following outputs until next user question).
