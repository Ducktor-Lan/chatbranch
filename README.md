...
5. **多格式导出**
   - 可选择某个问题编号，仅导出该问题及其对应输出块
   - 支持 Markdown、HTML、JSON、纯文本四种格式
...
1. 打开 ChatGPT / Gemini / DeepSeek 对话页
2. 使用右侧索引面板搜索并跳转历史提问
3. 分支提问：
   - 选中文本
...
- `Ctrl+Shift+B`（Mac: `Cmd+Shift+B`）：切换面板显示
- `Ctrl+Shift+Q`（Mac: `Cmd+Shift+Q`）：打开快速提问对话框
- `Ctrl+Shift+F`（Mac: `Cmd+Shift+F`）：聚焦搜索框
...
2. **Branch Ask (New Tab Workflow)**
   ...
   - **Preview**: Preview full prompt before sending, edit question in real-time

3. **Prompt Library**
   - One button opens prompt library modal
   - Custom title and content for each prompt
   - Click title to insert content
   - Add, view, delete, and reuse saved prompt templates
   - Category filter (General/Writing/Coding/Translation/Analysis)
...
1. Open a ChatGPT, Gemini, or DeepSeek conversation page
...
- `Ctrl+Shift+B` (Mac: `Cmd+Shift+B`): Toggle panel visibility
- `Ctrl+Shift+Q` (Mac: `Cmd+Shift+Q`): Open quick ask dialog
- `Ctrl+Shift+F` (Mac: `Cmd+Shift+F`): Focus search input
...
**新功能：**
- **深色模式**：面板自动跟随系统主题切换深色/浅色模式
- **提示词库增强**：
  - 分类筛选（通用/写作/编程/翻译/分析）
  - 搜索功能（按标题、内容搜索）
  - 导入/导出 JSON 文件，方便备份和分享
- **全局快捷键**：
  - `Ctrl+Shift+B`（Mac: `Cmd+Shift+B`）：切换面板显示
  - `Ctrl+Shift+Q`（Mac: `Cmd+Shift+Q`）：打开快速提问对话框
  - `Ctrl+Shift+F`（Mac: `Cmd+Shift+F`）：聚焦搜索框
  - 可在 `chrome://extensions/shortcuts` 自定义
- **导出多格式**：支持 Markdown、HTML、JSON、纯文本四种格式导出
- **分支提问预览**：发送前预览完整 prompt，可编辑问题内容

**改进：**
- 提示词数据结构升级，支持唯一 ID、分类、创建时间
- 导出对话框 UI 优化，支持预览选择的消息标题
...
**新功能：**
- 自定义消息名称：悬停在索引项上可点击编辑按钮自定义名称，名称与会话绑定存储
- 提示词标题支持：提示词库现在支持设置标题和内容分离，点击标题插入完整内容
- 设置页面重构：提示词管理改为双列表格形式，更直观易用

**改进：**
- 提示词数据格式升级为 `{ title, content }` 结构，旧数据自动兼容转换
- 改进自定义滚动容器的跳转逻辑，提升非 window 滚动场景下的定位准确性
