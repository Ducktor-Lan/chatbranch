(function registerQwen(global) {
  const siteKey = "qwen";
  const utils = global.ChatBranchAdapterUtils;

  function match(locationObj) {
    return locationObj.hostname === "www.qianwen.com";
  }
  ...
  function getRole(el) {
    ...
    if (/assistant|ai|model|answer|qwen|tongyi/i.test(roleAttr)) {
      return "assistant";
    }
    ...
    if (token.includes("assistant") || token.includes("qwen") || token.includes("tongyi") || token.includes("ai")) {
      return "assistant";
    }

    const text = utils.cleanMessageText(el.innerText || "");
    if (/^我[：:]/.test(text) || /^我想|^请帮|^帮我|^给我|^请你/.test(text)) {
      return "user";
    }
